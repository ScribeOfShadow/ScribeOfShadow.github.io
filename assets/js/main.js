...
loader.load('assets/models/brass_goblets.glb', (gltf) => {
  ...
  // Choose a goblet (assumes the first one is fine)
  const goblet = goblets.children[0]; // adjust this if needed

  // Create a "blood" cylinder
  const bloodGeo = new THREE.CylinderGeometry(0.1, 0.1, 0.01, 32);
  const bloodMat = new THREE.MeshStandardMaterial({ color: 0x880000, roughness: 0.3, metalness: 0.1 });
  const blood = new THREE.Mesh(bloodGeo, bloodMat);
  blood.position.set(0, -0.3, 0); // adjust to inside the goblet
  scene.add(blood);
